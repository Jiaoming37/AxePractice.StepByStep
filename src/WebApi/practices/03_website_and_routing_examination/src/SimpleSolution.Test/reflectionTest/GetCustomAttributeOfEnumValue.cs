...
namespace CSharpViaTest.OtherBCLs.HandleReflections
{
    ...
    static class EnumDescriptionExtension
    {
        ...
        public static string GetDescription<T>(this T value)
        {
            ...
            if (!value.GetType().IsEnum) throw new NotSupportedException();

            var fieldInfo = value.GetType().GetField(value.ToString());
            var attribute = (MyEnumDescriptionAttribute)Attribute.GetCustomAttribute(fieldInfo, typeof(MyEnumDescriptionAttribute), false);
