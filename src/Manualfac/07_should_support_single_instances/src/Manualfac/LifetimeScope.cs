...
namespace Manualfac
{
    public class LifetimeScope : Disposable, ILifetimeScope
    {
        ...
        public object GetCreateShare(ComponentRegistration registration)
        {
            ...
            var containsKey = sharedInstances.ContainsKey(registration.Service);


            object result = containsKey ? sharedInstances[registration.Service]
                                        : registration.Activator.Activate(this);

            if (registration.Sharing == InstanceSharing.Shared && !containsKey)
            {
                sharedInstances.Add(registration.Service, result);
            }
